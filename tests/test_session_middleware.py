...
class TestSessionAwareToolHandler:
    ...
    def test_init(self, mock_mcp_instance) -> None:
        ...
        assert hasattr(handler, "original_tool_handlers")

    @patch("mistmcp.session_middleware.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_wrap_tool_enabled(self, mock_is_enabled, tool_handler) -> None:
    ...
    @patch("mistmcp.session_middleware.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_wrap_tool_disabled(self, mock_is_enabled, tool_handler) -> None:
    ...
    def test_apply_session_middleware(self, tool_handler, mock_mcp_instance) -> None:
        """Test applying session middleware to tools"""
        # Mock tool objects with handlers
        for _, tool_obj in getattr(mock_mcp_instance, "_tools").items():
        ...
        assert "getOrgs" in tool_handler.original_tool_handlers
        assert "getSites" in tool_handler.original_tool_handlers

        # Verify handlers have been replaced
        tool = getattr(mock_mcp_instance, "_tools")["getOrgs"]
        tool = getattr(mock_mcp_instance, "_tools")["getSites"]

        # The handler should have been replaced with wrapped version
        assert tool.handler != tool_handler.original_tool_handlers["getOrgs"]
        assert tool.handler != tool_handler.original_tool_handlers["getSites"]
    ...
    def test_apply_session_middleware_no_handler_attr(
        ...
        for tool_obj in getattr(mock_mcp_instance, "_tools").values():
    ...
    def test_get_available_tools_with_session(
        self, mock_session_manager, tool_handler
    ) -> None:
        """Test getting available tools when session exists"""
        # Mock session with specific enabled tools
        mock_session = Mock()
        mock_session.enabled_tools = {"getSelf", "manageMcpTools", "getOrgs"}
        ...
        assert "getOrgs" in available_tools
        assert "getSites" not in available_tools
    ...
    def test_get_available_tools_no_session(
        ...
        assert "getSelf" in available_tools
        assert "manageMcpTools" in available_tools
        assert "getOrgs" not in available_tools
        assert "getSites" not in available_tools

    def test_get_available_tools_no_tools_attr(
        self, tool_handler, mock_mcp_instance
    ) -> None:
...
class TestCreateSessionAwareMcpServer:
    ...
    def test_create_session_aware_server(self) -> None:
        ...
        for tool_obj in getattr(base_server, "_tools").values():
            tool_obj.handler = AsyncMock()
        ...
        assert isinstance(
            getattr(result_server, "_session_handler"), SessionAwareToolHandler
        )
        ...
        assert getattr(result_server, "_session_handler").mcp_instance is base_server

    def test_create_session_aware_server_middleware_applied(self) -> None:
        ...
        original_handler = getattr(base_server, "_tools")["getOrgs"].handler

        # Create session-aware server
        result_server = create_session_aware_mcp_server(base_server)

        # Verify middleware was applied (handler should be wrapped)
        current_handler = getattr(base_server, "_tools")["getOrgs"].handler
        ...
        assert (
            ...
            or len(getattr(result_server, "_session_handler").original_tool_handlers)
            > 0
        )
