...
class TestSessionTool:
    ...
    @pytest.mark.asyncio
    async def test_session_tool_enabled(self, mock_is_enabled) -> None:
    ...
    @patch("mistmcp.session_tools.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_session_tool_disabled(self, mock_is_enabled) -> None:
    ...
    def test_session_tool_metadata(self) -> None:
        ...
        assert getattr(test_tool, "_is_session_tool") is True
        assert hasattr(test_tool, "_mcp_tool_config")
        assert getattr(test_tool, "_mcp_tool_config")["enabled"] is True
        assert getattr(test_tool, "_mcp_tool_config")["description"] == "Test tool"
        assert getattr(test_tool, "_mcp_tool_config")["category"] == "test"
        assert hasattr(test_tool, "_original_func")

    def test_session_tool_default_params(self) -> None:
        ...
        assert getattr(test_tool, "_is_session_tool") is True
        assert hasattr(test_tool, "_mcp_tool_config")
        assert getattr(test_tool, "_mcp_tool_config")["enabled"] is False

    @patch("mistmcp.session_tools.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_session_tool_with_args(self, mock_is_enabled) -> None:
        """Test session_tool decorator with function arguments"""
        mock_is_enabled.return_value = True

        @session_tool(enabled=True)
        async def test_tool(arg1, arg2, kwarg1=None) -> str:
...
class TestRequireSessionTool:
    ...
    @patch("mistmcp.session_tools.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_require_session_tool_enabled(self, mock_is_enabled) -> None:
        """Test require_session_tool when required tool is enabled"""
        mock_is_enabled.return_value = True

        @require_session_tool("getOrgs")
        async def helper_function() -> str:
            return "helper_result"

        result = await helper_function()
        assert result == "helper_result"
        mock_is_enabled.assert_called_once_with("getOrgs")

    @patch("mistmcp.session_tools.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_require_session_tool_disabled(self, mock_is_enabled) -> None:
        ...
        error = exc_info.value
        assert "requires the 'getOrgs' tool" in str(error)
        mock_is_enabled.assert_called_once_with("getOrgs")

    @patch("mistmcp.session_tools.is_tool_enabled_for_current_session")
    @pytest.mark.asyncio
    async def test_require_session_tool_with_args(self, mock_is_enabled) -> None:
...
class TestGetSessionToolInfo:
    ...
    def test_get_session_tool_info_with_session_tool(self) -> None:
    ...
    def test_get_session_tool_info_with_regular_function(self) -> None:
    ...
    def test_get_session_tool_info_with_partial_metadata(self) -> None:
        """Test getting info from function with partial session tool metadata"""

        # Add partial metadata
        async def partial_tool() -> str:
            return "success"

        # Add partial metadata
        setattr(partial_tool, "_is_session_tool", True)
    ...
    def test_get_session_tool_info_false_flag(self) -> None:
        """Test getting info from function with _is_session_tool = False"""

        mock_tool = Mock()
        setattr(mock_tool, "_is_session_tool", False)
        setattr(mock_tool, "_mcp_tool_config", {"enabled": True})

        info = get_session_tool_info(mock_tool)
        assert info is None
...
class TestRegisterSessionToolWithMcp:
    ...
    def test_register_session_tool_success(self) -> None:
    ...
    def test_register_session_tool_with_regular_function(self) -> None:
    ...
    def test_register_session_tool_with_empty_config(self) -> None:
    ...
    def test_register_session_tool_with_complex_config(self) -> None:
