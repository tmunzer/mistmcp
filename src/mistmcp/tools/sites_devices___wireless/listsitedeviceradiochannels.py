...
def add_tool() -> None:
    mcp.add_tool(
        fn=listSiteDeviceRadioChannels,
        name="listSiteDeviceRadioChannels",
        description="""Get a list of allowed channels (per channel width)""",
        tags={"Sites Devices - Wireless"},
        annotations={
            "title": "listSiteDeviceRadioChannels",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("listSiteDeviceRadioChannels")


async def listSiteDeviceRadioChannels(
    ...
    response = mistapi.api.v1.sites.devices.listSiteDeviceRadioChannels(
        apisession,
        site_id=str(site_id),
