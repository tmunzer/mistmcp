...
def add_tool() -> None:
    mcp.add_tool(
        fn=countSiteWanClientEvents,
        name="countSiteWanClientEvents",
        description="""Count by Distinct Attributes of Site WAN Client-Events""",
        tags={"Sites Clients - Wan"},
        annotations={
            "title": "countSiteWanClientEvents",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("countSiteWanClientEvents")


async def countSiteWanClientEvents(
    ...
    response = mistapi.api.v1.sites.wan_client.countSiteWanClientEvents(
        apisession,
        site_id=str(site_id),
