...
def add_tool() -> None:
    mcp.add_tool(
        fn=searchSiteWanClients,
        name="searchSiteWanClients",
        description="""Search Site WAN Clients""",
        tags={"Sites Clients - Wan"},
        annotations={
            "title": "searchSiteWanClients",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("searchSiteWanClients")


async def searchSiteWanClients(
    ...
    response = mistapi.api.v1.sites.wan_clients.searchSiteWanClients(
        apisession,
        site_id=str(site_id),
        mac=mac,
        hostname=hostname,
