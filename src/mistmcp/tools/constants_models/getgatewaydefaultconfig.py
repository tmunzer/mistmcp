...
def remove_tool():
    mcp.remove_tool("getGatewayDefaultConfig")

async def getGatewayDefaultConfig(
    model: Annotated[str, Field(description="""Model the default gateway config is intended (as the default LAN/WAN port can differ)""")],
    ha: Annotated[Optional[str], Field(description="""Whether the config is intended for HA""")] | None = None,
) -> dict:
    """Generate Default Gateway Config"""

    response = mistapi.api.v1.const.default_gateway_config.getGatewayDefaultConfig(
            apisession,
            model=model,
            ha=ha,
    )
