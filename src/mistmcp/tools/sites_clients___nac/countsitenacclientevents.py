...
def add_tool():
    mcp.add_tool(
        fn=countSiteNacClientEvents,
        name="countSiteNacClientEvents",
        description="""Count by Distinct Attributes of NAC Client-Events""",
        tags={"Sites Clients - NAC"},
        annotations={
            "title": "countSiteNacClientEvents",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("countSiteNacClientEvents")

async def countSiteNacClientEvents(
    ...
    response = mistapi.api.v1.sites.nac_clients.countSiteNacClientEvents(
            apisession,
            site_id=str(site_id),
