...
async def searchOrgNacClients(
    ...
    auth_type: Annotated[Optional[str], Field(description="""Authentication type, e.g. 'eap-tls', 'eap-peap', 'eap-ttls', 'eap-teap', 'mab', 'psk', 'device-auth'""")],
    edr_managed: Annotated[Optional[bool], Field(description="""Filters NAC clients that are integrated with EDR providers""")],
    edr_provider: Annotated[Optional[Edr_provider], Field(description="""EDR provider of client's organization""")] = Edr_provider.NONE,
    edr_status: Annotated[Optional[Edr_status], Field(description="""EDR Status of the NAC client""")] = Edr_status.NONE,
    family: Annotated[Optional[str], Field(description="""Client family, e.g. 'Phone/Tablet/Wearable', 'Access Point'""")],
    hostname: Annotated[Optional[str], Field(description="""Client hostname, e.g. 'my-laptop', 'my-phone'""")],
    idp_id: Annotated[Optional[str], Field(description="""SSO ID, if present and used""")],
    mac: Annotated[Optional[str], Field(description="""MAC address""")],
    mdm_compliance: Annotated[Optional[str], Field(description="""MDM compliance of client i.e 'compliant', 'not compliant'""")],
    mdm_provider: Annotated[Optional[str], Field(description="""MDM provider of client’s organization eg 'intune', 'jamf'""")],
    mdm_managed: Annotated[Optional[bool], Field(description="""Filters NAC clients that are managed by MDM providers""")],
    mfg: Annotated[Optional[str], Field(description="""Client manufacturer, e.g. 'apple', 'cisco', 'juniper'""")],
    model: Annotated[Optional[str], Field(description="""Client model, e.g. 'iPhone 12', 'MX100'""")],
    nacrule_name: Annotated[Optional[str], Field(description="""NAC Policy Rule Name matched""")],
    nacrule_id: Annotated[Optional[str], Field(description="""NAC Policy Rule ID, if matched""")],
    nacrule_matched: Annotated[Optional[bool], Field(description="""NAC Policy Rule Matched""")],
    ...
    ingress_vlan: Annotated[Optional[str], Field(description="""Vendor specific Vlan ID in radius requests""")],
    os: Annotated[Optional[str], Field(description="""Client OS, e.g. 'iOS 18.1', 'Android', 'Windows', 'Linux'""")],
    ssid: Annotated[Optional[str], Field(description="""SSID""")],
    status: Annotated[Optional[Status], Field(description="""Connection status of client i.e 'permitted', 'denied, 'session_stared', 'session_ended'""")] = Status.NONE,
    text: Annotated[Optional[str], Field(description="""partial / full MAC address, last_username, device_mac, nas_ip or last_ap""")],
    timestamp: Annotated[Optional[float], Field(description="""Start time, in epoch""")],
    type: Annotated[Optional[str], Field(description="""Client type i.e. 'wireless', 'wired' etc.""")],
    usermac_label: Annotated[Optional[list], Field(description="""Labels derived from usermac entry""")],
    username: Annotated[Optional[str], Field(description="""Username presented by the client""")],
    vlan: Annotated[Optional[str], Field(description="""Vlan name or ID assigned to the client""")],
    site_id: Annotated[Optional[str], Field(description="""Site id if assigned, null if not assigned""")],
    limit: Optional[int],
