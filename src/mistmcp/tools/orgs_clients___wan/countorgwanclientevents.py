...
def add_tool():
    mcp.add_tool(
        fn=countOrgWanClientEvents,
        name="countOrgWanClientEvents",
        description="""Count by Distinct Attributes of Org WAN Client-Events""",
        tags={"Orgs Clients - Wan"},
        annotations={
            "title": "countOrgWanClientEvents",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("countOrgWanClientEvents")

async def countOrgWanClientEvents(
    ...
    response = mistapi.api.v1.orgs.wan_client.countOrgWanClientEvents(
            apisession,
            org_id=str(org_id),
