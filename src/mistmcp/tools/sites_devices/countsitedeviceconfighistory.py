...
def add_tool():
    mcp.add_tool(
        fn=countSiteDeviceConfigHistory,
        ...
        tags={"Sites Devices"},
        annotations={
            "title": "countSiteDeviceConfigHistory",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("countSiteDeviceConfigHistory")

async def countSiteDeviceConfigHistory(
    ...
    response = mistapi.api.v1.sites.devices.countSiteDeviceConfigHistory(
            apisession,
            site_id=str(site_id),
            distinct=distinct,
            mac=mac,
