...
def add_tool() -> None:
    mcp.add_tool(
        fn=exportSiteDevices,
        name="exportSiteDevices",
        description="""To download the exported device information""",
        tags={"Sites Devices"},
        annotations={
            "title": "exportSiteDevices",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("exportSiteDevices")


async def exportSiteDevices(
    site_id: Annotated[UUID, Field(description="""ID of the Mist Site""")],
) -> dict:
    """To download the exported device information"""

    response = mistapi.api.v1.sites.devices.exportSiteDevices(
        apisession,
        site_id=str(site_id),
    )
