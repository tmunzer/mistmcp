...
def add_tool():
    mcp.add_tool(
        fn=countSiteWirelessClientEvents,
        name="countSiteWirelessClientEvents",
        description="""Count by Distinct Attributes of Client-Events""",
        tags={"Sites Clients - Wireless"},
        annotations={
            "title": "countSiteWirelessClientEvents",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("countSiteWirelessClientEvents")

async def countSiteWirelessClientEvents(
    ...
    proto: Annotated[Proto, Field(description="""a / b / g / n / ac / ax""")] = Proto.NONE,
    ...
    response = mistapi.api.v1.sites.clients.countSiteWirelessClientEvents(
            apisession,
            site_id=str(site_id),
