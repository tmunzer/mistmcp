...
def add_tool():
    mcp.add_tool(
        fn=getSiteWxTunnel,
        name="getSiteWxTunnel",
        description="""Get Site WxLan tunnel Details""",
        tags={"Sites WxTunnels"},
        annotations={
            "title": "getSiteWxTunnel",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("getSiteWxTunnel")

async def getSiteWxTunnel(
    site_id: Annotated[UUID, Field(description="""ID of the Mist Site""")],
    wxtunnel_id: Annotated[UUID, Field(description="""ID of the Mist Wxtunnel""")],
) -> dict:
    """Get Site WxLan tunnel Details"""

    response = mistapi.api.v1.sites.wxtunnels.getSiteWxTunnel(
            apisession,
            site_id=str(site_id),
            wxtunnel_id=str(wxtunnel_id),
    )
