...
def add_tool():
    mcp.add_tool(
        fn=getSiteSleClassifierDetails,
...
def remove_tool():
    mcp.remove_tool("getSiteSleClassifierDetails")

async def getSiteSleClassifierDetails(
    ...
    classifier: Annotated[str, Field(description="""Values from listSiteSlesMetrics""")],
    start: Annotated[Optional[int], Field(description="""Start datetime, can be epoch or relative time like -1d, -1w; -1d if not specified""")] ,
    end: Annotated[Optional[int], Field(description="""End datetime, can be epoch or relative time like -1d, -2h; now if not specified""")]  ,
