...
async def listOrgNacTags(
    ...
    name: Annotated[Optional[str], Field(description="""Name of NAC Tag""")] = None,
    match: Annotated[
        Match,
        Field(
    ...
    nactag_id: Annotated[
        Optional[UUID],
        Field(
            description="""ID of the NAC Tag to filter by. Providing this parameter will return only the specified object and may provide additional information."""
        ),
    ] = None,
) -> dict:
    ...
    if nactag_id:
        response = mistapi.api.v1.orgs.nactags.getOrgNacTag(
            apisession, org_id=str(org_id), nactag_id=str(nactag_id)
        )
    else:
