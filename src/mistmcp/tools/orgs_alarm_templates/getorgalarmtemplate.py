...
def add_tool():
    mcp.add_tool(
        fn=getOrgAlarmTemplate,
        name="getOrgAlarmTemplate",
        description="""Get Org Alarm Template Details""",
        tags={"Orgs Alarm Templates"},
        annotations={
            "title": "getOrgAlarmTemplate",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("getOrgAlarmTemplate")

async def getOrgAlarmTemplate(
    org_id: Annotated[UUID, Field(description="""ID of the Mist Org""")],
    alarmtemplate_id: Annotated[UUID, Field(description="""ID of the Mist Alarmtemplate""")],
) -> dict:
    ...
    response = mistapi.api.v1.orgs.alarmtemplates.getOrgAlarmTemplate(
            apisession,
            org_id=str(org_id),
            alarmtemplate_id=str(alarmtemplate_id),
    )
