...
def add_tool():
    mcp.add_tool(
        fn=listOrgAssetsStats,
        name="listOrgAssetsStats",
        description="""Get List of Org Assets Stats""",
        tags={"Orgs Stats - Assets"},
        annotations={
            "title": "listOrgAssetsStats",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("listOrgAssetsStats")

async def listOrgAssetsStats(
    ...
    response = mistapi.api.v1.orgs.stats.listOrgAssetsStats(
            apisession,
            org_id=str(org_id),
            start=start,
            end=end,
