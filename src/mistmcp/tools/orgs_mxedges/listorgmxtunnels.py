...
async def listOrgMxTunnels(
    ...
    mxtunnel_id: Annotated[
        Optional[UUID],
        Field(
            description="""ID of the MX Tunnel to filter by. Providing this parameter will return only the specified object and may provide additional information."""
        ),
    ] = None,
) -> dict:
    ...
    if mxtunnel_id:
        response = mistapi.api.v1.orgs.mxtunnels.getOrgMxTunnel(
            apisession, org_id=str(org_id), mxtunnel_id=str(mxtunnel_id)
        )
    else:
