...
def add_tool() -> None:
    mcp.add_tool(
        fn=countOrgSiteMxEdgeEvents,
        name="countOrgSiteMxEdgeEvents",
        description="""Count by Distinct Attributes of Org Mist Edge Events""",
        tags={"Orgs MxEdges"},
        annotations={
            "title": "countOrgSiteMxEdgeEvents",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("countOrgSiteMxEdgeEvents")


async def countOrgSiteMxEdgeEvents(
    ...
    response = mistapi.api.v1.orgs.mxedges.countOrgSiteMxEdgeEvents(
        apisession,
        org_id=str(org_id),
