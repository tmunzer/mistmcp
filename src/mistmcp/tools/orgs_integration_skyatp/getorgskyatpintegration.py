...
def add_tool():
    mcp.add_tool(
        fn=getOrgSkyAtpIntegration,
        name="getOrgSkyAtpIntegration",
        description="""Get Org SkyATP Integration""",
        tags={"Orgs Integration SkyATP"},
        annotations={
            "title": "getOrgSkyAtpIntegration",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("getOrgSkyAtpIntegration")

async def getOrgSkyAtpIntegration(
    org_id: Annotated[UUID, Field(description="""ID of the Mist Org""")],
) -> dict:
    """Get Org SkyATP Integration"""

    response = mistapi.api.v1.orgs.setting.getOrgSkyAtpIntegration(
            apisession,
            org_id=str(org_id),
    )
