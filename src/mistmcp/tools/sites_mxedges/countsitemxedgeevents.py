...
def add_tool() -> None:
    mcp.add_tool(
        fn=countSiteMxEdgeEvents,
        name="countSiteMxEdgeEvents",
        description="""Count by Distinct Attributes of Mist Edge Events""",
        tags={"Sites MxEdges"},
        annotations={
            "title": "countSiteMxEdgeEvents",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("countSiteMxEdgeEvents")


async def countSiteMxEdgeEvents(
    ...
    response = mistapi.api.v1.sites.mxedges.countSiteMxEdgeEvents(
        apisession,
        site_id=str(site_id),
