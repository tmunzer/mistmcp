...
def add_tool():
    mcp.add_tool(
        fn=getOrgServicePolicy,
        name="getOrgServicePolicy",
        description="""Get Org Service Policy Details""",
        tags={"Orgs Service Policies"},
        annotations={
            "title": "getOrgServicePolicy",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("getOrgServicePolicy")

async def getOrgServicePolicy(
    org_id: Annotated[UUID, Field(description="""ID of the Mist Org""")],
    servicepolicy_id: Annotated[UUID, Field(description="""ID of the Mist Servicepolicy""")],
) -> dict:
    ...
    response = mistapi.api.v1.orgs.servicepolicies.getOrgServicePolicy(
            apisession,
            org_id=str(org_id),
            servicepolicy_id=str(servicepolicy_id),
    )
