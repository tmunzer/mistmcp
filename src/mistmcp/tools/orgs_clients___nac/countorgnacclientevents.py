...
def add_tool():
    mcp.add_tool(
        fn=countOrgNacClientEvents,
        name="countOrgNacClientEvents",
        description="""Count by Distinct Attributes of NAC Client-Events""",
        tags={"Orgs Clients - NAC"},
        annotations={
            "title": "countOrgNacClientEvents",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("countOrgNacClientEvents")

async def countOrgNacClientEvents(
    ...
    response = mistapi.api.v1.orgs.nac_clients.countOrgNacClientEvents(
            apisession,
            org_id=str(org_id),
