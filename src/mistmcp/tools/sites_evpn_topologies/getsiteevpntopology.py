...
def add_tool():
    mcp.add_tool(
        fn=getSiteEvpnTopology,
        name="getSiteEvpnTopology",
        description="""Get One EVPN Topology Detail""",
        tags={"Sites EVPN Topologies"},
        annotations={
            "title": "getSiteEvpnTopology",
            "readOnlyHint": True,
...
def remove_tool():
    mcp.remove_tool("getSiteEvpnTopology")

async def getSiteEvpnTopology(
    site_id: Annotated[UUID, Field(description="""ID of the Mist Site""")],
    evpn_topology_id: Annotated[UUID, Field(description="""ID of the Mist Evpn_topology""")],
) -> dict:
    ...
    response = mistapi.api.v1.sites.evpn_topologies.getSiteEvpnTopology(
            apisession,
            site_id=str(site_id),
            evpn_topology_id=str(evpn_topology_id),
    )
