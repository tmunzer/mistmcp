...
def add_tool() -> None:
    mcp.add_tool(
        fn=listAlarmDefinitions,
        name="listAlarmDefinitions",
        description="""Get List of brief definitions of all the supported alarm types. The example field contains an example payload as you would receive in the alarm webhook output.HA cluster node names will be specified in the `node` field, if applicable.""",
        tags={"Constants Definitions"},
        annotations={
            "title": "listAlarmDefinitions",
            "readOnlyHint": True,
...
def remove_tool() -> None:
    mcp.remove_tool("listAlarmDefinitions")


async def listAlarmDefinitions() -> dict:
