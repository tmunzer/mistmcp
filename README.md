...
|----------|-------------|-------|-----------|
| **clients** | Clients related objects for the sites and organizations. | 13 | `searchOrgWirelessClientEvents`, `searchOrgWirelessClients`, `searchOrgWirelessClientSessions`, `listOrgGuestAuthorizations`, `searchOrgGuestAuthorization`, `searchOrgNacClientEvents`, `searchOrgNacClients`, `searchOrgWanClientEvents`, `searchOrgWanClients`, `searchOrgWiredClients`, `listSiteAllGuestAuthorizations`, `listSiteAllGuestAuthorizationsDerived`, `searchSiteGuestAuthorization` |
...
| **devices** | Devices are any Network device managed or monitored by Juniper Mist. | 12 | `searchOrgDeviceEvents`, `listOrgApsMacs`, `searchOrgDevices`, `listOrgDevicesSummary`, `getOrgInventory`, `searchOrgInventory`, `getOrgJuniperDevicesCommand`, `listSiteDevices`, `searchSiteDeviceConfigHistory`, `searchSiteDeviceEvents`, `searchSiteDeviceLastConfigs`, `searchSiteDevices` |
...
| **mxedges** | MX Edge related objects for the organizations. It provides access to Mist Edges, Mist Clusters, and Mist Tunnels. | 8 | `listOrgMxEdgeClusters`, `listOrgMxEdges`, `searchOrgMistEdgeEvents`, `searchOrgMxEdges`, `getOrgMxEdgeUpgradeInfo`, `listOrgMxTunnels`, `listSiteMxEdges`, `searchSiteMistEdgeEvents` |
...
| **orgs_nac** | NAC related objects for the organizations. It provides access to NAC Endpoints, NAC fingerprints, tags, and rules. | 4 | `listOrgNacRules`, `listOrgNacTags`, `searchOrgUserMacs`, `searchOrgClientFingerprints` |
| **orgs_sites** | tools to Create or Get the Organization Sites. | 3 | `listOrgSiteGroups`, `searchOrgSites`, `listOrgSiteTemplates` |
...
| **orgs_stats** | Statistics for the organizations. It provides access to various statistics related to the organization, such as BGP peers, devices, MX edges, other devices, ports, sites, tunnels, and VPN peers. | 9 | `getOrgStats`, `searchOrgBgpStats`, `listOrgDevicesStats`, `listOrgMxEdgesStats`, `getOrgOtherDeviceStats`, `searchOrgSwOrGwPorts`, `listOrgSiteStats`, `searchOrgTunnelsStats`, `searchOrgPeerPathStats` |
...
| **templates_wan** | WAN Configuration related objects for the sites and organizations. | 17 | `listOrgAAMWProfiles`, `listOrgAntivirusProfiles`, `listOrgGatewayTemplates`, `listOrgIdpProfiles`, `listOrgNetworks`, `listOrgSecPolicies`, `listOrgServicePolicies`, `listOrgServices`, `listOrgVpns`, `listSiteApps`, `listSiteGatewayTemplateDerived`, `listSiteNetworksDerived`, `listSiteSecIntelProfilesDerived`, `listSiteServicePoliciesDerived`, `listSiteServicesDerived`, `searchSiteServicePathEvents`, `listSiteVpnsDerived` |
...
| **webhooks** | A Webhook is a configuration that allows real-time events and data from the Org to be pushed to a provided url. | 4 | `listOrgWebhooks`, `searchOrgWebhooksDeliveries`, `listSiteWebhooks`, `searchSiteWebhooksDeliveries` |
